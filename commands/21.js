...
if(channels.some(a => message.channel.id === a)) return message.lineReply(`**Não é permitido usar comandos nesse chat**`).then(m => m.delete({timeout: 30000})).catch(a => {})
    ...
    let moneydb = await db.get(`money_${message.guild.id}_${user.id}`)
      ...
      if (!money || money < 1 || money > moneydb) {
          message.lineReply("Coloque um número válido.")
          return
      }

      if (!moneydb) {
          message.lineReply("Você não tem dinheiro")
          return
      }
      ...
      function endMsg(title, msg, dealerC) {
          ...
          player.cards.forEach(function(card) {
              ...
              cardsMsg += "`](https://discord.gg/92sz2vTF5g) "
          });
          ...
          if (!dealerC) {
              ...
              dealerMsg += " ? ?`](https://discord.gg/92sz2vTF5g)"
          } else {
              dealerMsg = "";
      ...
      async function loop() {
          ...
          endMsg("21", '**Coloque ``c`` Para comprar, e ``p`` para parar o jogo!** ', false)
          ...
          message.channel.awaitMessages(filter, {
              ...
              if (message.content === "c" || message.content === "C") {
                hit();
                loop();
                return
            } else if (message.content === "p" || message.content === "P") {
                stand();
                loop();
                return
            } else {
              ...
              message.lineReply("**Você perdeu tudo que tinha**");
              bet("lose");
              return
          })
      }
