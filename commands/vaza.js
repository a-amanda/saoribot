...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        try {
            if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("**🚨 | Desculpe, mas você não tem permissão para isso! - [BAN_MEMBERS]**");
            if (!message.guild.me.hasPermission("BAN_MEMBERS")) return message.channel.send("**🚨 | Desculpe, mas eu não tenho permissão para isso!- [BAN_MEMBERS]**");
            if (!args[0]) return message.channel.send("**Forneça um usuário para banir!**")

            let banMember = message.mentions.members.first() || message.guild.members.cache.get(args[0]) || message.guild.members.cache.find(r => r.user.username.toLowerCase() === args[0].toLocaleLowerCase()) || message.guild.members.cache.find(ro => ro.displayName.toLowerCase() === args[0].toLocaleLowerCase());
            if (!banMember) return message.channel.send("**O usuário não está na guilda**");
            if (banMember === message.member) return message.channel.send("**Você não pode se banir**")

            var reason = args.slice(1).join(" ");

            if (!banMember.bannable) return message.channel.send("**Não consigo chutar esse usuário**")
            try {
